...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Verificar se o novo nickname já existe para outro utilizador
    $stmt_check = $ligacao->prepare("SELECT id FROM users WHERE nickname = ? AND id != ?");
    $stmt_check->bind_param("si", $nickname, $user_id);
    $stmt_check->execute();
    ...
    if ($stmt_check->num_rows > 0) {
        ...
        if (isset($_FILES['photo-upload']) && $_FILES['photo-upload']['error'] == 0) {
            ...
            // Garante um nome de ficheiro único
            $file_extension = pathinfo($_FILES["photo-upload"]["name"], PATHINFO_EXTENSION);
            $target_file = $upload_dir . $user_id . '_' . time() . '.' . $file_extension;
        ...
        if ($message_type !== 'error') {
            // Atualizar dados na base de dados (sem o email)
            $stmt = $ligacao->prepare("UPDATE users SET nickname = ?, fullName = ?, dob = ?, photo_path = ? WHERE id = ?");
            $stmt->bind_param("ssssi", $nickname, $fullName, $dob, $photo_path, $user_id);

            if ($stmt->execute()) {
                $message = "Perfil atualizado com sucesso!";
                $message_type = 'success';
            } else {
                $message = "Erro ao atualizar o perfil: " . $stmt->error;
                $message_type = 'error';
            }
            $stmt->close();
        }
...
<head>
    ...
    <style>
        ...
        .profile-form input:disabled, .profile-form input[readonly] {
    ...
    <div id="side-menu" class="side-menu">
        ...
        <nav>
            <ul>
                ...
                <li><a href="carrinho.php"><i class="fas fa-shopping-cart"></i>Carrinho <span id="menu-cart-badge" class="cart-badge">0</span></a></li>
                ...
                <form class="profile-form" id="profile-form" method="POST" action="perfil.php" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                     ...
                     <div class="form-group">
                        ...
                        <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($utilizador['email'] ?? ''); ?>" readonly>
                    </div>
                     <div class="form-group">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function toggleFormEdit(enable) {
                formInputs.forEach(input => {
                    if (input.id !== 'email') { // O email nunca é editável
